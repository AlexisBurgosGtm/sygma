...
router.post("/checkin_verify", async(req,res)=>{
   
    const { token, sucursal,fecha,codemp } = req.body;

    let qry = `
        SELECT 
            HORA_INICIO, 
            ISNULL(HORA_FIN,'NO') AS HORA_FIN 
        FROM 
            EMPLEADOS_CHECK_IN
...
router.post("/checkin_inicio", async(req,res)=>{
   
    const { token, sucursal,fecha,hora,codemp,lat,long } = req.body;

    let qry = `
        INSERT INTO EMPLEADOS_CHECK_IN (EMPNIT,CODEMP,FECHA,HORA_INICIO, LAT_INICIO, LONG_INICIO)
        SELECT '${sucursal}' AS EMPNIT, ${codemp} AS CODEMP, '${fecha}' AS FECHA, 
                '${hora}' AS HORA_NICIO, ${lat} AS LAT_INICIO, ${long} AS LONG_INICIO;
            `
    
    execute.QueryToken(res,qry,token); 
     
});
router.post("/checkin_final", async(req,res)=>{
...
router.post("/empleados_listado", async(req,res)=>{
    ...
    let qry = `
        SELECT        
            ...
            EMPLEADOS.CODRUTA, 
            EMPLEADOS.CODCATALOGO, 
            ISNULL(RUTAS_CLIENTES.DESRUTA,'') AS DESRUTA
        FROM  EMPLEADOS LEFT OUTER JOIN
            RUTAS_CLIENTES ON EMPLEADOS.CODRUTA = RUTAS_CLIENTES.CODRUTA AND EMPLEADOS.EMPNIT = RUTAS_CLIENTES.EMPNIT LEFT OUTER JOIN
