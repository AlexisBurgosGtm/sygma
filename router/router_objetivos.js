...
router.post("/get_cobertura_vendedores", async (req, res) => {
    const { token, sucursal, mes, anio } = req.body;
    ...
    const qry = `
        ...
        SELECT
            ...
            ISNULL(UE.UNIVERSO_EMP, 0) AS UNIVERSO_EMP,
            ISNULL(IPE.IMPORTE, 0) AS IMPORTE,
            (SELECT COUNT(*) FROM ClientesAlcanzados) AS TOTAL_CLIENTES
        FROM EmpleadosActivos EA
        LEFT JOIN ClientesPorEmpleado CPE
            ON EA.CODEMP = CPE.CODEMP
        LEFT JOIN ImportePorEmpleado IPE
...
router.post("/get_cobertura_marcas_por_vendedor", async (req, res) => {
    const { token, sucursal, mes, anio, codemp } = req.body;
    const codempNum = Number(codemp) || 0;
    ...
    const qry = `
        SELECT
            ...
            M.DESMARCA,
            COUNT(DISTINCT CASE
                WHEN CL.CODEMPLEADO = ${codempNum} THEN D.CODCLIENTE
            END) AS CLIENTES,
            SUM(CASE
                WHEN D.CODEMP = ${codempNum} THEN ISNULL(DP.TOTALPRECIO, 0)
                ELSE 0
            END) AS IMPORTE,
            U.UNIVERSO
        FROM DOCUMENTOS D
        ...
        WHERE D.STATUS <> 'A'
            ...
            AND D.CODCLIENTE > 0
            AND (
                CL.CODEMPLEADO = ${codempNum}
                OR D.CODEMP = ${codempNum}
            )
        GROUP BY M.CODMARCA, M.DESMARCA, U.UNIVERSO
        HAVING
            COUNT(DISTINCT CASE WHEN CL.CODEMPLEADO = ${codempNum} THEN D.CODCLIENTE END) > 0
            OR SUM(CASE WHEN D.CODEMP = ${codempNum} THEN ISNULL(DP.TOTALPRECIO, 0) ELSE 0 END) > 0
        ORDER BY IMPORTE DESC, M.DESMARCA
    `;
