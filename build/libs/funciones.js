let F = {
      ...
      fcn_solicitar_fel(coddoc,correlativo,nit,nombre,direccion,municipio,departamento,idbtn,fecha){
          ...
          nit = F.limpiarTexto(nit).toUpperCase();

          F.Confirmacion('¿Está seguro que desea intentar certificar esta factura?')
          .then((value)=>{
            if(value==true){
                  ...
                  F.getXmlFel(coddoc,correlativo,nit,nombre,direccion,municipio,departamento,fecha)
                  .then((xmlstring)=>{
                    
                          F.converBase64(xmlstring)
                          .then((valor)=>{
                                F.solicitar_FEL(coddoc,correlativo,valor)
                                .then((data)=>{
                                    if(data.resultado==true){
                                        //console.log(data);
                                        F.enviar_FEL_firmado(coddoc,correlativo,data.archivo)
                                        .then((data)=>{
                                                //console.log(data);
                                                if(data.resultado==true){
                                                  F.Aviso('Factura firmada exitosamente!!');

                                                  btnCertif.innerHTML = '<i class="fal fa-print"></i>SOLICITAR FACTURA';
                                                  btnCertif.disabled = false;
                                                  
                                                  F.update_FEL_series(coddoc,correlativo,data.uuid.toString(),data.serie.toString(),data.numero.toString(),data.fecha.toString())
                                                  .then(()=>{
                                                      apigen.pedidosVendedor(GlobalCodSucursal,GlobalCodUsuario,F.devuelveFecha('txtFecha'),'tblReport','containerTotal');
                                                  })
                                                  .catch(()=>{
                                                  ...
                                                  F.mostrarErrores(JSON.stringify(data.descripcion_errores));

                                                  F.AvisoError('No se pudo crear la factura');
                                                }
                                        })
                                        .catch((error)=>{
                                          console.log('Error: ');
                                      ...
                                      F.AvisoError('Factura no se pudo certificar');

                                      F.mostrarErrores(data.descripcion);
                                ...
                                .catch((error)=>{
                                    ...
                                    btnCertif.innerHTML = '<i class="fal fa-print"></i>SOLICITAR FACTURA';
                                    btnCertif.disabled = false;
                                    F.AvisoError('Error al certificar')
                  ...
                  .catch((err)=>{
                      btnCertif.innerHTML = '<i class="fal fa-print"></i>SOLICITAR FACTURA';
                      btnCertif.disabled = false;
                      F.AvisoError('No se pudo obtener el xml')
                  })
      ...
      fcn_solicitar_fel_directo(coddoc,correlativo,nit,nombre,direccion,municipio,departamento,idbtn,fecha){
          ...
          return new Promise((resolve,reject)=>{
            ...
            if(FEL.CONFIG_FEL_HABILITADO=='NO'){reject();F.AvisoError('No está habilitado para emitir Factura Electrónica');return;};
      ...
      getXmlFel(coddoc,correlativo,nit,nombre,direccion,municipio,departamento,fecha){
          ...
          return new Promise((resolve,reject)=>{
            let fechaemision = F.get_FEL_fecha(fecha); //'2022-11-27T10:49:22.000-06:00';
            ...
            .then((response) => {
                ...
                data.map((rows)=>{
                        ...
                        totaliva += Number(iva);
                        strdata += F.getStrItem(numerolinea,rows.CANTIDAD,rows.CODMEDIDA,rows.DESPROD,rows.PRECIO,0,subtotal,iva);
                })
